...
**A. ByteDance iframe 拼图**（同进程 Playwright，主路径）
    solve_slider_captcha, wait_for_slider_captcha_ui_ready, page_suggests_captcha_iframe_or_images

**B. 易盾 yidun 拼图**（子进程 / 独立命令行，见 :func:`run_yidun_cli`）
...
# ByteDance 类验证通过色约为 #52C41A 或 #00C1A7（青绿）
...
def _visible_text_blob_frames(page: Any) -> str:
...
def _wait_for_pass(
    ...
    while time.time() < deadline:
        ...
        try:
            blob = _visible_text_blob_frames(page)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  B. 易盾（yidun）拼图 — 供 risk_verification_command 子进程调用
# ═══════════════════════════════════════════════════════════════════════════════


def _risk_page_url_from_environ() -> str:
    return (
        os.environ.get("RISK_VERIFICATION_URL", "")
        or os.environ.get("KR36_RISK_URL", "")
        or ""
    ).strip()


def _headed_load_wait_ms() -> int:
    raw = (
        os.environ.get("PLAYWRIGHT_HEADED_LOAD_WAIT_MS")
        or os.environ.get("KR36_HEADED_LOAD_WAIT_MS")
        or "120000"
    )
    return max(3000, int(raw or "120000"))


def _yidun_wait_for_page_ready_before_headed_close(page: Any, *, headless: bool) -> None:
    if headless:
        return
    try:
        ...
        return
    max_ms = _headed_load_wait_ms()
    print("[yidun] agent-browser：等待 load → networkidle 后再关闭窗口…", flush=True)
    try:
    ...
    try:
        ...
        print(
            "[yidun] 等待 networkidle 超时，继续关闭。",
            file=sys.stderr,
            flush=True,
        )


def _yidun_load_cv2_np():
    try:
        import cv2  # type: ignore[import-untyped]
        import numpy as np  # type: ignore[import-untyped]
    except ImportError as exc:
        print(
            "[yidun] 需要 opencv-python-headless 与 numpy。"
...
def _yidun_piece_to_bgr(cv2: Any, np: Any, piece: Any) -> Any:
...
def yidun_match_slide_x(cv2: Any, np: Any, bg_bgr: Any, piece_bgr: Any) -> tuple[int, float]:
    """在背景上匹配拼图块，返回最佳匹配左上角 x 与得分。"""
    piece_bgr = _yidun_piece_to_bgr(cv2, np, piece_bgr)
...
def yidun_tracks_for_distance(distance: float) -> tuple[list[float], list[float]]:
...
def _yidun_natural_size(locator: Any) -> tuple[int, int]:
...
def _yidun_find_bg_frame(page: Any):
    from playwright.sync_api import TimeoutError as PlaywrightTimeoutError
...
def _yidun_pick_drag_handle(frame: Any):
    ordered = [
...
def _yidun_captcha_still_present(page: Any) -> bool:
    for frame in page.frames:
        try:
...
def run_yidun_solver(
    *,
    ...
    try:
        ...
        print("[yidun] 未安装 playwright。", file=sys.stderr)
        raise SystemExit(2) from exc
    ...
    with sync_playwright() as p:
        ...
        def close_browser() -> None:
            _yidun_wait_for_page_ready_before_headed_close(page, headless=headless)
            browser.close()

        for round_i in range(1, max(1, max_rounds) + 1):
            if not _yidun_captcha_still_present(page):
                snapshot = page.content()
                if _looks_like_captcha_or_block(snapshot):
                    print(
                        "[yidun] 页面仍像拦截/验证页，但未发现易盾拼图。"
                        "退出码 3。",
                        file=sys.stderr,
                    )
                    close_browser()
                    return 3
                print("[yidun] 页面上已无易盾拼图，认为通过。")
                close_browser()
                return 0

            frame, bg_loc = _yidun_find_bg_frame(page)
            if frame is None or bg_loc is None:
                print("[yidun] 未找到易盾背景图节点。", file=sys.stderr)
                close_browser()
                return 1

            piece_sel = frame.locator("img.yidun_jigsaw, img[class*='yidun_jigsaw']")
            if piece_sel.count() == 0:
                print("[yidun] 缺少拼图块 img.yidun_jigsaw。", file=sys.stderr)
                close_browser()
                return 1
            ...
            try:
                ...
                print(f"[yidun] 读取图片地址失败: {exc}", file=sys.stderr)
                close_browser()
                return 1

            if not bg_src or not pc_src:
                print("[yidun] 背景或拼图 src 为空。", file=sys.stderr)
                close_browser()
                return 1
            ...
            try:
                ...
                print(f"[yidun] 下载验证图片失败: {exc}", file=sys.stderr)
                close_browser()
                return 1
            ...
            if bg_bgr is None or pc_bgr is None:
                print("[yidun] 图片解码失败。", file=sys.stderr)
                close_browser()
                return 1

            mx, score = yidun_match_slide_x(cv2, np, bg_bgr, pc_bgr)
            print(f"[yidun] 模板匹配 x={mx} score={score:.4f} (round {round_i})")

            nat_w, _ = _yidun_natural_size(bg_loc)
            box = bg_loc.bounding_box()
            if not box or not nat_w:
                print("[yidun] 无法读取背景图布局尺寸。", file=sys.stderr)
                close_browser()
                return 1
            scale = float(box["width"]) / float(nat_w)
            drag_px = mx * scale

            handle = _yidun_pick_drag_handle(frame)
            if handle is None:
                print("[yidun] 未找到可拖动把手。", file=sys.stderr)
                close_browser()
                return 1

            hbox = handle.bounding_box()
            if not hbox:
                print("[yidun] 把手不可见。", file=sys.stderr)
                close_browser()
                return 1
            ...
            page.mouse.down()
            plus, minus = yidun_tracks_for_distance(drag_px)
            acc_x = cx
            for dx in plus:
                acc_x += dx
        ...
        print("[yidun] 多轮尝试后仍存在拼图。", file=sys.stderr)
        close_browser()
        return 1


def run_yidun_cli() -> None:
    """``python -m utils.slider_solver.captcha`` 或 ``python -m utils.slider_solver`` 入口。"""
    parser = argparse.ArgumentParser(
        description="易盾拼图滑块：读取 RISK_VERIFICATION_URL（兼容 KR36_RISK_URL）。",
    )
    parser.add_argument(
        "--url",
        default=_risk_page_url_from_environ(),
        help="验证页 URL（默认环境变量 RISK_VERIFICATION_URL，兼容 KR36_RISK_URL）",
    )
    ...
    if not args.url:
        print(
            "[yidun] 缺少 URL：请设置 RISK_VERIFICATION_URL（或兼容 KR36_RISK_URL）或传入 --url。",
            file=sys.stderr,
        )
        raise SystemExit(2)
    code = run_yidun_solver(
        url=args.url,
