...
def handle_analyze_content_command(args: argparse.Namespace, *, paths: AppPaths, facade: Any) -> None:
    ...
    for target_date in target_dates:
        ...
        brief_markdown = facade.generate_brief_markdown(payload, llm_client, checkpoint_store=step6_checkpoint_store)
        html_output, step6_log = write_step6_outputs(
            brief_output=output_paths.brief_output,
            markdown_text=brief_markdown,
            run_dir=run_dir,
            target_date=target_date,
            context="c114-analyze-content builtin",
        )
        print(f"主题简报 MD: {output_paths.brief_output}")
        print(f"主题简报 HTML: {html_output}")
...
def _handle_controller_analyze_content_command(
    *,
    ...
    if not output_paths.brief_output.exists():
        template_markdown = facade.render_brief_markdown(payload)
        write_step6_outputs(
            brief_output=output_paths.brief_output,
            markdown_text=template_markdown,
            run_dir=run_dir,
            target_date=target_date,
            context="c114-analyze-content controller-agent template",
        )
    manifest_path = facade.write_controller_agent_manifest(
        day_dir=run_dir,
        target_date=target_date,
