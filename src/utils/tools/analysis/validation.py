...
from .step5_recovery_notes import note_step5_recovery
...
def normalize_content_analysis_topic_response(
    ...
    模型偶发漏字段或与输入标题对不齐时，跳过无法对齐的条目并打日志，避免整批失败；
    若本批输入中没有任何一条能映射到模型结果，仍抛错以便上层重试。
    """
    ...
    for raw_item in raw_items:
        ...
        if not original_title:
            note_step5_recovery(f"step 5 跳过模型返回项：original_title 为空（主题={category.topic}）。")
            continue
    ...
    for item in category.items:
        if not str(item.original_title).strip():
            note_step5_recovery(f"step 5 跳过输入条目：original_title 为空（主题={category.topic}）。")
            continue
        ...
        if analysis is None:
            note_step5_recovery(
                "step 5 跳过输入条目：模型结果中未匹配到 "
                f"original_title={item.original_title!r}（主题={category.topic}）。"
            )
            continue
    ...
    if not completed_items:
        raise StructuredLLMError(
            "step 5 主题分析结果未能与任何输入条目对齐；请重试或检查模型输出。"
        )
