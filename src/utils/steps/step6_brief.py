...
from utils.tools.analysis.models import BRIEF_PROMPT_PATH, BriefSectionDraft, ContentAnalysisInput, ContentAnalysisSection
from utils.tools.analysis.yaml_io import load_content_analysis_inputs
from utils.tools.brief.markdown import render_brief_markdown, render_generated_brief_markdown
from utils.tools.brief.render import render_layer_issues_yaml
from utils.tools.facades.content import load_search_results_yaml
from utils.tools.facades.intelligence import load_daily_articles_from_csv
...
from utils.tools.search.workflow import load_search_checklist_yaml
        ...
        try:
            section_text = complete_json_with_postprocess_retry(
                ...
                user_prompt=(
                    ...
                    "并在「核心判断」中按系统提示分层：大标题用「一、」「二、」，小观点用「1、」「2、」+ 核心结论：一段展开（勿用主观点/解释标签，勿用 1） 括号编号）。"
