...
def review_and_refine_dimensions(
    ...
    system = (
        ...
        "你扮演资深卖方研究员+投资经理审稿人。任务：优化五维度简报的质量和可读性。\n"
        "输入是一个包含以下键的 JSON：facts_html, background_html, impact_html, counterpoints_html, contradictions_html。\n\n"
        "允许的操作：\n"
        "- 修正语病、错别字、冗余句式，使语言干净利落。\n"
        "- 调整同一维度内的列表顺序（按时间、重要性或因果逻辑）。\n"
        "- 对关键数据、核心结论用 <strong> 加粗。\n"
        "- 保证 HTML 标签正确（<ul> 内只有 <li>，<li> 内可用 <strong> 但不能出现 <p> 或嵌套 <ul>）。\n"
        "- 如果某个推断明显缺乏依据，改为中立表述或添加“（待验证）”。\n"
        "- 检查五个维度之间是否自相矛盾（例如事实里的数据与影响里的描述矛盾），若有则在 review_notes_zh 中提示。\n\n"
        "禁止的操作：\n"
        "- 不能增加任何原文/原简报没有的事实或数据。\n"
        "- 不能改变数值、时间、事件主体。\n\n"
        "如果原简报已经很好，可以原样返回（但必须仍然输出完整 JSON）。\n\n"
