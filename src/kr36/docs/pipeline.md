...
| 2 | 拉取原文 | `kr36_step2_content_YYYYMMDD.yaml` | **仅**根据步骤 1 中的**文章链接**拉取页面正文，不经过 C114 的搜索清单、不外搜补链。实现见 `utils.steps.kr36_synthetic_workflow_from_step1` + 正文抓取。**正文「文章行」条数与步骤 1 一致**：`build_synthetic_workflow_from_analyses` 内校验 `sum(每主题 items) == len(analyses)`；`run_content_fetch_workflow(..., expected_article_count=len(analyses))` 在抓取前后再校验，避免静默丢行。 |
...
`websearch` / `c114` 的 `--source 36kr` 会调用 `kr36.cli:run_with_args`：完成步骤 1 的 CSV 落盘后，在 **不写入** `kr36_step_2_search_checklist_*.yaml` 的前提下，直接调用 `kr36.core.pipeline:run_stages_2_3_4`。
...
会打印日志并调用 `utils.slider_solver.kr36_bridge.try_36kr_step4_html_after_risk` → `Kr36SourceAdapter.step4_fetch_html_via_playwright_slider`（Chromium + 自动滑块）。

## 相关源码

- `src/kr36/core/pipeline.py`：步骤 2–4 编排
- `src/utils/steps/kr36_synthetic_workflow_from_step1.py`：用 `ArticleAnalysis` 构造与「无外搜」等价的拉取计划
- `src/kr36/core/names.py`：文件名约定
- `src/kr36/emailing/preview_assets.py`：由 Step6 Markdown 生成 HTML / 纯文本 / doc 预览（内部调用模板渲染）
- `src/kr36/emailing/template.py` + `src/kr36/emailing/email_renderer_impl.py`：固定三栏（专题 / 活动 / 资讯）邮件与预览 HTML 的排版与 step5 对齐逻辑
