...
def _llm_trace_log_name_for_step(step_name: str, report_date: str) -> str:
    """生成按步骤拆分的 LLM 日志文件名。"""

    normalized_step = str(step_name).strip() or "unknown"
    normalized_date = report_date.replace("-", "")
    return f"c114_llm_trace_{normalized_step}_{normalized_date}.jsonl"
...
class StructuredChatClient:
    ...
    def set_trace_log_directory(
        ...
        *,
        ...
        with self._trace_log_lock:
            ...
            if reset_files:
                pattern = _llm_trace_log_name_for_step("*", report_date)
    ...
    def _resolved_trace_log_path(self, step_name: str) -> Path | None:
        ...
        if self._trace_log_dir is None or not self._trace_log_date:
            return None
        return self._trace_log_dir / _llm_trace_log_name_for_step(step_name, self._trace_log_date)
