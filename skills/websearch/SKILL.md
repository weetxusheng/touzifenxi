---
name: websearch
description: 当用户需要抓取、搜索、分析并审查 C114 或 InfoQ 当日热点时使用。该 skill 由 Python 内置主备模型自动完成 Step 1.5、Step 2、Step 3、Step 5、Step 6、Step 7。
---

# Websearch 当日热点 Skill

## 适用场景

- 用户希望抓取 C114 首页、指定栏目或 InfoQ 当天可见文章
...
1. 初始化本地配置模板：
  - `python scripts/websearch.py c114-config-init`
2. 编辑：
  - `config/runtime.local.json`
3. 检查缺项：
  - `python scripts/websearch.py c114-config-status`
4. 至少确认：
  - 执行模式：
    - `execution.mode`
...
- 默认主路径：
  - `python scripts/websearch.py run --date YYYY-MM-DD`
