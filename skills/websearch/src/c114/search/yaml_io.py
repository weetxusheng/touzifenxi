...
def save_search_results(output_path: Path, payload: SearchWorkflowPayload) -> None:
    ...
    source_prefix = infer_source_prefix_from_output_path(output_path)
    stats_path = output_path.parent / provider_stats_name(date.fromisoformat(payload.report_date), source_prefix=source_prefix)
    stats_path.write_text(render_provider_usage_stats(payload), encoding="utf-8")
    review_view_path = output_path.parent / build_review_view_name(payload.report_date, source_prefix=source_prefix)
    review_view_path.write_text(render_search_review_view(payload), encoding="utf-8")


def infer_source_prefix_from_output_path(output_path: Path) -> str:
    """从 step 输出文件名推断站点前缀，供辅助产物命名复用。"""

    stem = output_path.stem
    if "_step_" in stem:
        prefix = stem.split("_step_", 1)[0].strip()
        if prefix:
            return prefix
    return "c114"


def build_review_view_name(report_date: str, *, source_prefix: str = "c114") -> str:
    normalized_prefix = str(source_prefix or "c114").strip().lower().replace("-", "_") or "c114"
    return f"{normalized_prefix}_search_review_view_{report_date.replace('-', '')}.yaml"
