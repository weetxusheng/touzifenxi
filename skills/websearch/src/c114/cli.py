...
def run_with_args(args: argparse.Namespace, paths: AppPaths | None = None) -> None:
    ...
    # 单 skill 模式下，InfoQ 不再通过独立 skill 执行，而是由同一个 run 入口分流。
    if args.command == "run" and getattr(args, "source", "c114") == "infoq":
        from infoq.cli import run_with_args as run_infoq_with_args

        infoq_dates = resolve_c114_date_range(args, default_to_today=True)
